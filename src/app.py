...
from src.sr_api import pga_schedule, pga_player_stats, wgr_rankings, tournament_scores_round
...
from src.calibrator import calibrate_global
...
if run:
    ...
    # Save projection snapshot for “learning”
    save_json({
        "year": int(year),
...
st.caption("When a tournament is complete, pull actual results and compare to your projections. This version provides diagnostics; you can evolve it into automatic weight updates.")
...
st.info(
    "Next step: parse final leaderboard/finishes from tournament summary or scores payload, "
    "then call a calibrator that updates global bias/variance and (later) feature weights."
)
